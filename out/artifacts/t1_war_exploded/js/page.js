/**
 * 对页面进行分页
 * @param obj 页码标签对象
 * @param  pageCount number 总页面数
 * @param  pageSize number 分页大小
 * @param currentPage number 当前页
 */
function PagingManage(obj, pageCount, currentPage) {
    if (obj) {
        var pagecount = pageCount; //页面总数
        var currentpage = currentPage;//当前页面
        var showPageNum = 6;//显示多少个页码

        var pagehtml = "";
        var divId = "" + obj.attr('id');

        //只有一页内容
        if (pagecount <= 1) {
            pagehtml = "";
        }

        //大于一页内容
        if (pagecount > 1) {
            if (currentpage > 1) {
                pagehtml += '<li><a href="javascript:void(0);" onclick="switchPage(\'' + divId + '\',' + (currentpage - 1) + ')">«</a></li>';
            }
            else {
                pagehtml += '<li class="disabled"><span>«</span></li>';
            }

            var startPage = 1;
            //计算页码开始位置
            if (showPageNum > pagecount) {//如果要显示的页码大于总的页码数
                startPage = 1
            } else {//如果要显示的页码小于总的页码数
                if (currentpage - (showPageNum / 2) <= 0) {//如果当前页面
                    startPage = 1;
                    console.log('currentpage - (showPageNum / 2) < 0');
                } else if (currentpage + (showPageNum / 2) >= pagecount) {
                    startPage = pagecount - showPageNum + 1;
                    console.log('currentpage + (showPageNum / 2) > pageNum');
                } else {
                    startPage = currentpage - (showPageNum / 2);
                }
            }

            startPage = parseInt(startPage);

            for (var i = startPage; i < (startPage + showPageNum); i++) {

                //如果要输出的页面大于总的页面数,则退出
                if (i > pagecount) {
                    break;
                }

                if (i === currentpage) {
                    pagehtml += '<li class="active"><a href="javascript:void(0);" onclick="switchPage(\'' + divId + '\',' + i + ')"><span>' + i + '</span></a></li>';
                } else {
                    pagehtml += '<li><a href="javascript:void(0);" onclick="switchPage(\'' + divId + '\',' + i + ')">' + i + '</a></li>';
                }
            }


            if (currentpage < pagecount) {
                pagehtml += '<li><a href="javascript:void(0);" onclick="switchPage(\'' + divId + '\',' + (currentpage + 1) + ')">»</a></li>';
            }
            else {
                pagehtml += '<li class="disabled"><span>»</span></li>';
            }
        }
        obj.html(pagehtml);
    }
}